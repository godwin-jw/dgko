...
export default function ListePage() {
  ...
  useEffect(() => {
    try {
      ...
      if (kayit) {
        setMaddeler(JSON.parse(kayit))
      } else {
        setMaddeler(BASLANGIC)
      }
    } catch {
      setMaddeler(BASLANGIC)
    }
  ...
  return (
    <>
      <style>{`
        ...
        .check-btn:hover { background: rgba(184,132,58,0.15); }
        .check-btn.checked {
        ...
        .ekleyen-badge.o {
          background: rgba(140,122,104,0.12);
          color: var(--muted);
          border: 1px solid rgba(140,122,104,0.2);
        }
          ...
          <div className="fi3">
            {maddeler.map(madde => (
              <div key={madde.id} className={`madde-row ${madde.yapildi ? 'yapildi-row' : ''}`}>
                <button
                  className={`check-btn ${madde.yapildi ? 'checked' : ''}`}
                  onClick={() => toggleYapildi(madde.id)}
                  title={madde.yapildi ? 'geri al' : 'yapıldı'}
                >
                  {madde.yapildi ? '✓' : ''}
                </button>
                ...
                <button className="sil-btn" onClick={() => maddeSil(madde.id)} title="sil">
                  ×
                </button>
              </div>
            ))}
            ...
            {eklemeAcik ? (
              <div className="ekle-form">
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && maddeEkle()}
                  placeholder="ne yapmak istiyorsunuz?"
                  autoFocus
                />
                <div className="ekleyen-secim">
                  <button
                    className={`ekleyen-btn ${yeniEkleyen === 'ben' ? 'aktif' : ''}`}
                    onClick={() => setYeniEkleyen('ben')}
                  >
                    nurettin
                  </button>
                  <button
                    className={`ekleyen-btn ${yeniEkleyen === 'o' ? 'aktif' : ''}`}
                    onClick={() => setYeniEkleyen('o')}
                  >
                    fidan
                  </button>
                </div>
                <div className="ekle-aksiyonlar">
                  <button className="ekle-iptal" onClick={() => { setEklemeAcik(false); setYeniMetin('') }}>
                    iptal
                  </button>
                  <button className="ekle-kaydet" onClick={maddeEkle} disabled={!yeniMetin.trim()}>
                    ekle →
                  </button>
                </div>
              </div>
            ) : (
              <button className="ekle-ac-btn" onClick={() => setEklemeAcik(true)}>
                + yeni madde ekle
              </button>
            )}
          </div>
