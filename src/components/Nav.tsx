  ...
  return (
    <>
      <style>{`
        ...
        .hamburger-btn.acik .hamburger-cizgi { background: var(--fg); }
        .hamburger-btn.acik .hamburger-cizgi:nth-child(1) { transform: translateY(6.5px) rotate(45deg); }
        .hamburger-btn.acik .hamburger-cizgi:nth-child(2) { opacity: 0; width: 0; }
        .hamburger-btn.acik .hamburger-cizgi:nth-child(3) { transform: translateY(-6.5px) rotate(-45deg); }
        .mobil-menu {
        ...
        .mobil-nav-link {
          ...
          font-weight: 300; font-size: 2rem; font-style: italic;
      ...
      <nav style={{
        ...
        <Link href="/" style={{
          ...
          fontSize: '1.1rem', color: 'var(--accent)', letterSpacing: '0.05em',
          textDecoration: 'none', zIndex: 60, position: 'relative',
        }}>f.</Link>
        ...
        {mobil && (
          <button className={`hamburger-btn ${acik ? 'acik' : ''}`} onClick={() => setAcik(a => !a)} aria-label="menü">
      ...
      {mobil && (
        <div className={`mobil-menu ${acik ? 'acik' : ''}`}>
          <nav style={{ display: 'flex', flexDirection: 'column' }}>
            {links.map(({ href, label }) => (
              <Link key={href} href={href} className={`mobil-nav-link ${pathname === href ? 'aktif' : ''}`}>
                {label}
              </Link>
            ))}
          </nav>
